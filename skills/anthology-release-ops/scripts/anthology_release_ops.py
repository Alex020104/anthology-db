...
# Базовые пути источников DB берутся от локальной папки игры релизера.
# Абсолютные пути внутри публичного update_rules.json намеренно не являются
# источником истины: у разных релизеров разные диски, а logical path должен быть
# одинаковым.
DB_SOURCE_DIRS = {
    "configs": LIVE_GAME_DIR / "db" / "configs",
    "mods": LIVE_GAME_DIR / "db" / "mods",
}
DB_SOURCE_DIRS.update({normalize_rel_path(key): db_source_dir_for(key, value) for key, value in DB_RULES.get("source_dirs", {}).items()})
...
DB_SOURCE_FILES.update({normalize_rel_path(key): db_source_file_for(key, value) for key, value in DB_RULES.get("source_files", {}).items()})

# Исключаем только явно запрещённые файлы. Если DB-файл есть у релизера в live
# папке и указан в правилах, он должен попасть в манифест, иначе mirror-режим
# может убрать его у игроков.
